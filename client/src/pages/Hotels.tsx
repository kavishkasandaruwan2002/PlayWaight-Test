...
const Hotels = () => {
    ...
    const handleBook = async () => {
        ...
        setIsBooking(true);
        try {
            const { data } = await api.post('/hotels/book', { hotelId: selectedHotel._id, date: bookingDate });
    ...
    return (
        <div className="min-h-screen py-32 px-6 max-w-7xl mx-auto w-full dark:bg-slate-950 luxury-mesh">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 gap-8" data-testid="hotel-loading-spinner">
                    <div className="w-20 h-20 border-[8px] border-blue-600/20 border-t-blue-600 rounded-full animate-spin" />
                    <p className="text-slate-500 font-black uppercase tracking-[0.3em] text-xs animate-pulse">Fetching Destinations...</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12" data-testid="hotel-grid">
                    {hotels.map((hotel, index) => (
                        <motion.div
                            ...
                            data-testid={`hotel-card-${hotel._id}`}
                        >
                            <div className="relative h-64 overflow-hidden">
                                ...
                                <div className="absolute top-6 right-6 glass px-4 py-2 rounded-2xl flex items-center gap-2 text-xs font-black tracking-widest uppercase text-white shadow-2xl border-white/20">
                                    <Star size={14} className="text-amber-400 fill-amber-400" />
                                    {hotel.rating}
                                </div>
                                <div className="absolute bottom-6 left-6 flex gap-2">
                                    <div className="p-2 glass rounded-xl text-white"><Wifi size={16} /></div>
                                    <div className="p-2 glass rounded-xl text-white"><Coffee size={16} /></div>
                                </div>
                            </div>
                            <div className="p-10 flex flex-col flex-1 space-y-8">
                                <div className="space-y-3">
                                    ...
                                    <h3 className="text-3xl font-black text-slate-950 dark:text-white truncate group-hover:text-blue-600 transition-colors tracking-tight" data-testid={`hotel-name-${index}`}>
                                        {hotel.name}
                                    </h3>
                                    <p className="text-slate-500 text-sm line-clamp-2 font-medium leading-relaxed">{hotel.description}</p>
                                </div>

                                <div className="mt-auto flex justify-between items-center border-t border-slate-100 dark:border-slate-800 pt-8">
                                    <div>
                                        <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest block mb-1">Per Night</span>
                                        <span className="text-3xl font-black text-slate-950 dark:text-white">${hotel.price}</span>
                                    </div>
                                    <button
                                        onClick={() => setSelectedHotel(hotel)}
                                        className="btn-primary px-8"
                                        data-testid={`book-hotel-btn-${index}`}
                                    >
                                        BOOK NOW
                                    </button>
                                </div>
            ...
            <AnimatePresence>
                {selectedHotel && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6" data-testid="booking-modal">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                        ...
                        >
                            ...
                            <div className="text-center mb-12 space-y-2">
                                <h2 className="text-4xl font-black text-slate-950 dark:text-white tracking-tighter">CONFIRM STAY</h2>
                                <p className="text-slate-500 font-medium italic">Your luxury experience at {selectedHotel.name}</p>
                            </div>
                            ...
                            <div className="space-y-10">
                                <div className="flex gap-8 p-8 bg-slate-50 dark:bg-slate-950 rounded-[40px] border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
                                    <img src={selectedHotel.image} alt={selectedHotel.name} className="w-32 h-32 rounded-3xl object-cover shadow-2xl relative z-10" />
                                    <div className="flex flex-col justify-center relative z-10">
                                        <p className="font-black text-2xl text-slate-950 dark:text-white tracking-tight">{selectedHotel.name}</p>
                                        <div className="flex items-center gap-2 text-blue-500 font-black text-[10px] uppercase tracking-[0.3em] mt-2">
                                            <MapPin size={12} /> {selectedHotel.location}
                                        </div>
                                        <p className="text-3xl font-black mt-4">${selectedHotel.price} <span className="text-sm font-medium text-slate-400">/ NIGHT</span></p>
                                    </div>
                                </div>

                                <div className="space-y-6">
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2">Arrival Date</label>
                                    ...
                                    <div className="p-6 bg-blue-600/5 rounded-3xl border border-blue-600/10 flex items-center gap-4">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm font-black text-slate-800 dark:text-slate-200">GUARANTEED BOOKING</p>
                                            <p className="text-[10px] text-slate-500 font-medium">Instant confirmation and premium support.</p>
                                        </div>
                                    </div>
                                </div>
